...
function getChild(el, childNum, options, includeDragEl) {
	...
	while (i < children.length) {
		if (
			children[i].style.display !== "none" && children[i] !== Sortable.ghost
			&& (includeDragEl || children[i] !== Sortable.dragged) && closest(children[i], options.draggable, el, false)
		) {
...
function lastChild(el, selector) {
	...
	while (
		last && (last === Sortable.ghost || css(last, "display") === "none" || selector && !matches(last, selector))
	) {
...
function isRectEqual(rect1, rect2) {
	return Math.round(rect1.top) === Math.round(rect2.top) && Math.round(rect1.left) === Math.round(rect2.left)
		&& Math.round(rect1.height) === Math.round(rect2.height) && Math.round(rect1.width) === Math.round(rect2.width);
}
		...
		animateAll: function animateAll(callback) {
			...
			animationStates.forEach(function(state) {
				...
				if (target.thisAnimationDuration) {
					...
					if (
						isRectEqual(prevFromRect, toRect) && !isRectEqual(fromRect, toRect)
...
function calculateRealTime(animatingRect, fromRect, toRect, options) {
	return Math.sqrt(Math.pow(fromRect.top - animatingRect.top, 2) + Math.pow(fromRect.left - animatingRect.left, 2))
...
var documentExists = typeof document !== "undefined",
	...
	_detectDirection = function _detectDirection(el, options) {
		...
		return child1
				&& (firstChildCSS.display === "block" || firstChildCSS.display === "flex"
					|| firstChildCSS.display === "table" || firstChildCSS.display === "grid"
	...
	_dragElInRowColumn = function _dragElInRowColumn(dragRect, targetRect, vertical) {
		...
		return dragElS1Opp === targetS1Opp || dragElS2Opp === targetS2Opp
		...
		function toFn(value, pull) {
			return function(to, from, dragEl, evt) {
				...
				if (value == null && (pull || sameGroup)) {
					...
					return value === true || typeof value === "string" && value === otherGroup
...
Sortable.prototype = /** @lends Sortable.prototype */ {
	...
	_prepareDragStart: function _prepareDragStart(/** Event */ evt, /** Touch */ touch, /** HTMLElement */ target) {
		...
		if (target && !dragEl && target.parentNode === el) {
			...
			if (
				options.delay && (!options.delayOnTouchOnly || touch)
	...
	_onTouchMove: function _onTouchMove(/**TouchEvent*/ evt) {
		if (tapEvt) {
			...
			if (ghostEl) {
				...
				var cssMatrix = "matrix(".concat(ghostMatrix.a, ",").concat(ghostMatrix.b, ",").concat(
					ghostMatrix.c,
					",",
				).concat(ghostMatrix.d, ",").concat(ghostMatrix.e, ",").concat(ghostMatrix.f, ")");
		...
		if (!ghostEl) {
			...
			if (PositionGhostAbsolutely) {
				...
				while (
					...
					&& css(ghostRelativeParent, "transform") === "none" && ghostRelativeParent !== document
				) {
	...
	_onDragOver: function _onDragOver(/**Event*/ evt) {
		...
		if (
			dragEl.contains(evt.target) || target.animated && target.animatingX && target.animatingY
			|| _this._ignoreWhileAnimating === target
		) {
		...
		ignoreNextClick = false;
		if (
			activeSortable && !options.disabled && (isOwner
...
function _getSwapDirection(
	...
	if (!invertSwap) {
		...
		if (isLastTarget && targetMoveDistance < targetLength * swapThreshold) {
			...
			if (
				!pastFirstInvertThresh && (lastDirection === 1
	...
	AutoScroll.prototype = {
		...
		_handleAutoScroll: function _handleAutoScroll(evt, fallback) {
			...
			if (fallback || this.options.forceAutoScrollFallback || Edge || IE11OrLess || Safari) {
				...
				if (
					!this.options.bubbleScroll || getParentAutoScrollElement(elem, true) === getWindowScrollingElement()
				) {
					clearAutoScrolls();
					return;
				}
...
var autoScroll = throttle(function(evt, options, rootEl, isFallback) {
	...
	do {
		...
		if (el === winScroller) {
			...
			canScrollY = height < scrollHeight
		...
		options.bubbleScroll && currentParent !== winScroller
...
var drop = function drop(_ref) {
	...
	hideGhostForTarget();
	var touch = originalEvent.changedTouches && originalEvent.changedTouches.length
	...
	MultiDrag.prototype = {
		...
		dragOverCompleted: function dragOverCompleted(_ref10) {
			...
			if (insertion) {
				...
				if (
					options.animation && multiDragElements.length > 1
