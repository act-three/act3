...
func TestBorderOutlineStates(t *testing.T) {
	v := hi.Transparent.Frame(hi.Width(80), hi.Height(40)).
		...
		WhileHovered(hi.BorderOutline(8, 7, hi.White)).
		BorderOutline(2, 1, hi.Black).
		BorderStroke(2, hi.Red).
		BorderShadow(0, 0, 10, 0, hi.Blue).
		BorderShape(hi.Capsule).Tag("button").Class("subject")
...
func TestBorderOutlineInvisiblePrecedence(t *testing.T) {
	for _, tt := range []struct {
		...
		{"transparent", hi.BorderOutline(4, 3, hi.Transparent), "3px", "oklch(0 0 0 / 0)"},
		{"zero width", hi.BorderOutline(4, 0, hi.Red), "0px", redCSS},
	} {
		t.Run(tt.name, func(t *testing.T) {
			stage(t, hi.Text("x").WhileFocused(tt.inner).
				BorderOutline(2, 1, hi.Blue).Tag("button").Class("subject"), func(s *uitest.Session) {
				var nodes []*cdp.Node
				s.Run(css.Enable(), chromedp.Nodes(".subject", &nodes, chromedp.ByQuery))
...
func TestBorderOutlineBrowserFocus(t *testing.T) {
	...
	// moving focus.
	v := hi.Text("x").Padding(hi.Edges(4)).
		WhilePressed(hi.BorderOutline(3, 2, hi.Red)).Tag("button").Class("subject")
	stage(t, v, func(s *uitest.Session) {
...
func TestBorderOutlineScopeAndStructure(t *testing.T) {
	...
	for _, width := range []complex128{0, 1, 3i} {
		for _, gap := range []complex128{0, 2, 5i} {
			for _, c := range []hi.Color{hi.Transparent, hi.Red} {
				v := base.BorderOutline(gap, width, c).
					WhileFocused(hi.BorderOutline(4, 3, hi.Blue)).BorderOutline(6, 5, hi.Red)
				html := render(t, v)
	...
	inner := hi.Text("x").BorderOutline(2, 1, hi.Red).Class("inner")
	stage(t, hi.HStack(inner).BorderOutline(4, 3, hi.Blue).Class("outer"), func(s *uitest.Session) {
		var got []string
