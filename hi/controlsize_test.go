...
func TestControlSizeEnvironment(t *testing.T) {
	...
	Render(VStack(
		...
		VStack(view(envProbe(&got[2])).ControlSize(Large)).ControlSize(Small),
		view(envProbe(&got[3])).ControlSize(Mini).ControlSize(Large),
		view(envProbe(&got[4])).Frame(Width(40)).ControlSize(Small),
	))
