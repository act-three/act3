...
func EdgeTop(px float64) EdgeSpace {
	checkLength(px)
	return EdgeSpace{Top: px}
}

// EdgeBottom sets the bottom edge to px pixels.
func EdgeBottom(px float64) EdgeSpace {
	checkLength(px)
	return EdgeSpace{Bottom: px}
}

// EdgeLeading sets the leading edge to px pixels.
func EdgeLeading(px float64) EdgeSpace {
	checkLength(px)
	return EdgeSpace{Leading: px}
}

// EdgeTrailing sets the trailing edge to px pixels.
func EdgeTrailing(px float64) EdgeSpace {
	checkLength(px)
	return EdgeSpace{Trailing: px}
}
