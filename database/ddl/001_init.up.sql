...
CREATE TABLE Episode (
	...
	-- can be empty if unaired/unreleased
...
CREATE TABLE SeasonEpisode (
	...
	SortKey TEXT NOT NULL,
	Label TEXT NOT NULL,
	-- episode number e.g. "5", or "Special"
	Number INTEGER,
	-- NULL for specials
...
CREATE TABLE Video (
	...
	-- empty during ingest
	MVPlaylist TEXT NOT NULL DEFAULT (''),
	-- empty during ingest
...
CREATE TABLE RenditionForStreaming (
	...
	VideoID TEXT NOT NULL REFERENCES Video,
	Remux INTEGER NOT NULL,
	-- 1: copy video stream; 0: reencode
	Codec TEXT NOT NULL,
	-- "h264" or "hevc"
	...
	-- 0 = source
	MaxFPS INTEGER NOT NULL DEFAULT (0),
	-- 0 = source
	CopyAudio INTEGER NOT NULL,
	-- 1: copy audio; 0: reencode to AAC
	SurroundAudio INTEGER NOT NULL DEFAULT (0),
	-- 1: encode as 5.1(back); 0: stereo downmix
...
CREATE TABLE DownloadPlan (
	...
	State TEXT NOT NULL DEFAULT 'downloading' CHECK (
		State IN ('downloading', 'downloaded', 'imported')
	),
	PRIMARY KEY (DownloadID, Path),
	CHECK (
		EpisodeID IS NULL
		OR MovieEditionID IS NULL
	)
) STRICT,
WITHOUT ROWID;
