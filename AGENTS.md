...
```sh
go build ./... # multiple packages avoids littering `act3` build artifact
go test ./...
go test ./model/...
...
Port 4445 belongs to the agent; port 4444 belongs to the user.
...
- **database/** — SQLite via `modernc.org/sqlite` (pure Go). Schema
  ...
  sqlc into `schema/`.
  See `go doc database` for policy.
  IDs are "flurry" IDs with per-table prefixes (`sr`, `ep`, `vid`, …).
  See `go doc database/flurry`.
...
Each generator is documented where it lives. Use `rg go:generate`
