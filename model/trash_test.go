...
func newTrashTestFixture(t *testing.T, m *Model) trashTestFixture {
	...
	err := m.WithTxRW(ctx, func(tx *TxRW) error {
		...
		fx.seriesID = sr.ID
		if err := tx.q.SlugClaim(schema.SlugClaimParams{
...
func TestTrashOrphanReapAcrossEditions(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.SeriesEdition{}, fx.edition1)
	}); err != nil {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.SeriesEdition{}, fx.edition2)
	}); err != nil {
...
func TestTrashDirectSkippedInCascade(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Season{}, fx.season1)
	}); err != nil {
	...
	if err := m.WithTxR(ctx, func(tx *TxR) error {
		st, err := tx.trashState(fx.season1)
		if err != nil {
			return err
		}
...
func TestTrashOrphanReapVideoSharedAcrossEpisodeAndMovie(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Episode{}, fx.episodeID)
	}); err != nil {
...
func TestTrashPurgeOrdering(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Series{}, seriesID)
	}); err != nil {
...
func TestTrashSlugCollisionOnRestore(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Movie{}, movieA)
	}); err != nil {
...
func TestTrashPurgeLeavesBlobsForGC(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Video{}, vidID)
	}); err != nil {
...
func TestTrashErrorCases(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Series{}, srID)
	}); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{
			"Trash non-existent",
			func(tx *TxRW) error { return tx.Trash(kind.Movie{}, missingID) },
			nil, // any non-nil error OK; not ErrAlreadyTrashed
		},
		{
			"Restore non-trashed",
			func(tx *TxRW) error { return tx.Restore(movieID) },
			ErrNotTrashed,
		},
		{
			"Restore non-existent",
			func(tx *TxRW) error { return tx.Restore(missingID) },
			nil,
		},
		{
			"Purge non-trashed",
			func(tx *TxRW) error { return tx.Purge(movieID) },
			ErrNotTrashed,
		},
		{
			"Purge non-existent",
			func(tx *TxRW) error { return tx.Purge(missingID) },
			nil,
		},
		{
			"Trash cascade-trashed episode",
			func(tx *TxRW) error { return tx.Trash(kind.Episode{}, cascadedEpID) },
			ErrAlreadyTrashed,
		},
		{
			"Trash invalid-kind ID",
			func(tx *TxRW) error { return tx.Trash(nil, invalidID) },
			nil,
		},
		{
			"Restore invalid-kind ID",
			func(tx *TxRW) error { return tx.Restore(invalidID) },
			nil,
		},
		{
			"Purge invalid-kind ID",
			func(tx *TxRW) error { return tx.Purge(invalidID) },
			nil,
		},
		{
			"Trash non-existent episode",
			func(tx *TxRW) error { return tx.Trash(kind.Episode{}, missingEpID) },
			sql.ErrNoRows,
		},
	}
...
func createMovieViaPublicAPI(t *testing.T, m *Model, title, releaseDate string) (string, error) {
	...
	var id string
	err := m.WithTxRW(ctx, func(tx *TxRW) error {
		mw, err := tx.MovieCreate(title, releaseDate)
		if err != nil {
			return err
		}
		id = mw.MovieHead.ID()
...
func TestTrashDefaultMovieEditionPromotesSuccessor(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		mw, err := tx.MovieEditionClone(ed1)
		if err != nil {
			return err
		}
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.MovieEdition{}, ed1)
	}); err != nil {
		t.Fatal(err)
	}
...
func TestTrashEpisodeTrashRestoreInMiddle(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Episode{}, targetEp)
	}); err != nil {
		t.Fatalf("trash: %v", err)
	}
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Restore(targetEp)
	}); err != nil {
...
func TestTrashRestoreCascadeTrashedErrors(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Series{}, fx.seriesID)
	}); err != nil {
		t.Fatal(err)
	}
	err := m.WithTxRW(ctx, func(tx *TxRW) error {
...
func TestTrashRestoreEpisodePullsUpTrashedSeries(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Episode{}, epID); err != nil {
			return err
		}
		return tx.Trash(kind.Series{}, srID)
	}); err != nil {
...
func TestTrashRoundTripMovieEdition(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.MovieEdition{}, medExtra)
	}); err != nil {
		t.Fatalf("trash: %v", err)
	}
...
func TestTrashRoundTripSeriesEdition(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.SeriesEdition{}, sedExtra)
	}); err != nil {
		t.Fatalf("trash: %v", err)
	}
...
func TestTrashRoundTripSeason(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Season{}, snA)
	}); err != nil {
...
func TestTrashRoundTripEpisode(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Episode{}, epID)
	}); err != nil {
		t.Fatalf("trash: %v", err)
	}
...
func TestTrashRoundTripCollection(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Collection{}, colID)
	}); err != nil {
		t.Fatalf("trash: %v", err)
	}
...
func TestTrashRestoreMEdPullsUpMovie(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.MovieEdition{}, medExtra); err != nil {
			return err
		}
		return tx.Trash(kind.Movie{}, movieID)
	}); err != nil {
...
func TestTrashRestoreSEdPullsUpSeries(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.SeriesEdition{}, sedExtra); err != nil {
			return err
		}
		return tx.Trash(kind.Series{}, srID)
	}); err != nil {
...
func TestTrashRestoreSeasonPullsUpSEd(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Season{}, snID); err != nil {
			return err
		}
		return tx.Trash(kind.SeriesEdition{}, sedExtra)
	}); err != nil {
...
func TestTrashRestoreEpisodePullsUpSeason(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		...
		return tx.Trash(kind.Season{}, snID)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
...
func TestTrashRestoreVideoPullsUpMEd(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Video{}, vidID); err != nil {
			return err
		}
		return tx.Trash(kind.MovieEdition{}, medExtra)
	}); err != nil {
...
func TestTrashRestoreEpisodeMultiHop(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		for _, target := range []struct {
			...
			{kind.Episode{}, epID},
			{kind.Season{}, snID},
			{kind.SeriesEdition{}, sedExtra},
			{kind.Series{}, srID},
		} {
			if err := tx.Trash(target.k, target.id); err != nil {
				return fmt.Errorf("trash %s: %w", target.id, err)
			}
		}
		return nil
	}); err != nil {
...
func TestTrashRestoreMovieInTrashedCollection(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Movie{}, movieID); err != nil {
			return err
		}
		return tx.Trash(kind.Collection{}, colID)
	}); err != nil {
...
func TestTrashRestoreSiblingsMovie(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Movie{}, mA); err != nil {
			return err
		}
		return tx.Trash(kind.Movie{}, mB)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Restore(mA)
	}); err != nil {
...
func TestTrashRestoreSiblingsEpisode(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Episode{}, ep1); err != nil {
			return err
		}
		return tx.Trash(kind.Episode{}, ep2)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Restore(ep1)
	}); err != nil {
...
func TestTrashRestoreJunctionSharedVideoMEdTrash(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.MovieEdition{}, med1)
	}); err != nil {
...
func TestTrashRestoreJunctionDanglingAutoRestore(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.Series{}, srID); err != nil {
			return err
		}
		return tx.Trash(kind.Movie{}, movieID)
	}); err != nil {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Restore(srID); err != nil {
			return err
		}
		return tx.Restore(movieID)
	}); err != nil {
...
func TestTrashOrphanReapAcrossSeries(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Series{}, sr1)
	}); err != nil {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Series{}, sr2)
	}); err != nil {
...
func TestTrashVideoAcrossTwoMEds(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.MovieEdition{}, med2)
	}); err != nil {
...
func TestTrashVideoAcrossTwoSeasons(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Season{}, sn1)
	}); err != nil {
...
func TestTrashCollectionSlugReuse(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Collection{}, c1)
	}); err != nil {
...
func TestTrashSlugGetAfterTrash(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Movie{}, mID)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxR(ctx, func(tx *TxR) error {
...
func TestTrashSlugCollisionOnRestoreCollection(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Collection{}, cA)
	}); err != nil {
		t.Fatal(err)
	}
	cB := createCollectionRow(t, m, "classics", "Classics")
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Restore(cA)
	}); err != nil {
...
func TestTrashDefaultSeriesEditionPromotesSuccessor(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.SeriesEdition{}, ed1)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxR(ctx, func(tx *TxR) error {
...
func TestTrashDefaultMovieEditionSole(t *testing.T) {
	...
	err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.MovieEdition{}, medID)
	})
...
func TestTrashDefaultSeriesEditionSole(t *testing.T) {
	...
	err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.SeriesEdition{}, sedID)
	})
...
func TestTrashDefaultSeriesEditionPromotesLexSmallest(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.SeriesEdition{}, edDefault)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxR(ctx, func(tx *TxR) error {
...
func TestPurgeSiblingSEd(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		if err := tx.Trash(kind.SeriesEdition{}, sed1); err != nil {
			return err
		}
		return tx.Trash(kind.SeriesEdition{}, sed2)
	}); err != nil {
		t.Fatal(err)
	}
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Purge(sed1)
	}); err != nil {
...
func TestTrashPurgeLoopMixedAges(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := m.WithTxRW(ctx, func(tx *TxRW) error {
			return tx.Trash(kind.Video{}, vidOld)
		}); err != nil {
			t.Fatal(err)
		}
		...
		if err := m.WithTxRW(ctx, func(tx *TxRW) error {
			return tx.Trash(kind.Video{}, vidNew)
		}); err != nil {
			t.Fatal(err)
		}
		if err := m.purgeTrashOnce(ctx); err != nil {
...
func TestTrashSortKeyBumpCollision(t *testing.T) {
	...
	if err := m.WithTxRW(ctx, func(tx *TxRW) error {
		return tx.Trash(kind.Episode{}, epIDs[1])
	}); err != nil {
		t.Fatal(err)
	}
